...
@WebMvcTest(controllers = { MatrixInvertController.class })
public class MatrixInvertControllerTest {

	@Autowired
	protected MockMvc mockMvc;

	@MockBean(name = "matrixInvertService")
	private MatrixOperationService matrixInvertService;

	@Test
	public void whenRequestIsValid() {
		try {
			BDDMockito.given(matrixInvertService.run(BDDMockito.any())).willReturn("1,4,7\n2,5,8\n3,6,9");
	...
	public void whenFileIsEmpty() {
		try {
			BDDMockito.given(matrixInvertService.run(BDDMockito.any()))
	...
	public void whenAnInternalError() {
		try {
			BDDMockito.given(matrixInvertService.run(BDDMockito.any())).willThrow(new InternalErrorException(
