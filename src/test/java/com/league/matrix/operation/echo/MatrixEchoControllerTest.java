...
@WebMvcTest(controllers = { MatrixEchoController.class })
public class MatrixEchoControllerTest {

	@Autowired
	protected MockMvc mockMvc;

	@MockBean(name = "matrixEchoService")
	private MatrixOperationService matrixEchoService;

	@Test
	public void whenRequestIsValid() {
		try {
			BDDMockito.given(matrixEchoService.run(BDDMockito.any())).willReturn("1,2,3\n4,5,6\n7,8,9");
	...
	public void whenFileIsEmpty() {
		try {
			BDDMockito.given(matrixEchoService.run(BDDMockito.any()))
	...
	public void whenAnInternalError() {
		try {
			BDDMockito.given(matrixEchoService.run(BDDMockito.any())).willThrow(new InternalErrorException(
