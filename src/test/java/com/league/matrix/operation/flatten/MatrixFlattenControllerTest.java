...
@WebMvcTest(controllers = { MatrixFlattenController.class })
public class MatrixFlattenControllerTest {

	@Autowired
	protected MockMvc mockMvc;

	@MockBean(name = "matrixFlattenService")
	private MatrixOperationService matrixFlattenService;

	@Test
	public void whenRequestIsValid() {
		try {
			BDDMockito.given(matrixFlattenService.run(BDDMockito.any())).willReturn("1,2,3,4,5,6,7,8,9");
	...
	public void whenFileIsEmpty() {
		try {
			BDDMockito.given(matrixFlattenService.run(BDDMockito.any()))
	...
	public void whenAnInternalError() {
		try {
			BDDMockito.given(matrixFlattenService.run(BDDMockito.any())).willThrow(new InternalErrorException(
