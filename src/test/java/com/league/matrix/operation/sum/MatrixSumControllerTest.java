...
@WebMvcTest(controllers = { MatrixSumController.class })
public class MatrixSumControllerTest {

	@Autowired
	protected MockMvc mockMvc;

	@MockBean(name = "matrixSumService")
	private MatrixOperationService matrixSumService;

	@Test
	public void whenRequestIsValid() {
		try {
			BDDMockito.given(matrixSumService.run(BDDMockito.any())).willReturn("45");
	...
	public void whenFileIsEmpty() {
		try {
			BDDMockito.given(matrixSumService.run(BDDMockito.any()))
	...
	public void whenAnInternalError() {
		try {
			BDDMockito.given(matrixSumService.run(BDDMockito.any())).willThrow(new InternalErrorException(
