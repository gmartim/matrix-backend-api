...
@Service("matrixInvertService")
@Slf4j
public class MatrixInvertService extends AbstractMatrixOperationService implements MatrixOperationService {

	/**
	 * Operates the matrix transposing it and after that joining the columns using a
	 * comma (",") and the rows using a new line ("\n").
	 * 
	...
	protected String operate(List<List<Integer>> matrix) throws OperationNotSupportedException, BadContentException {
		log.info("Started operating invert");

		List<List<Integer>> invertedMatrix;

		invertedMatrix = matrixActService.transpose(matrix);

		return matrixPrintService.join(invertedMatrix, COMMA, NEW_LINE);
	}
